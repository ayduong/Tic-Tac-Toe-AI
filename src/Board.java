...
public class Board {
	...
	public Board copyBoard() {
		...
		for(int i = 0; i < moves.size(); i++) {
				Move m = moves.get(i);
			newList.add(new Move(m.getRow(), m.getCol(), m.getValue()));
		}
	...
	/**
	 * This will remove the value from the board and add the move into the list of
	 * available moves.
	 * 
	...
	/**
	 * This gets all the moves. We make copies of the moves to avoid clobbering the
	 * values. We adjust the moves before giving out the list so that they have all
	 * the correct X's and Oh values.
	 *
	 * 0 = empty: no move 1 = X value 2 = O value
	 * 
